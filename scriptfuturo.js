...
function gerarFrase4() {
    ...
    // Construa a frase
    let frase4 = `Em atenção a reclamação ${numeroProtocolo3}, contatamos o(a) Sr.(a) ${nometit} no dia ${datacontato} ás ${horacontato} hs no número ${telcontato}, através de ${meiodecontato}. Identificamos que consumidor permanece com falha no serviço de ${servreparado}
    e realizamos o agendamento de reparo em seu serviço para data ${dataReparo} no perído da ${repprot}.`;
...
function calculoProrata() {
    ...
    // Criando a frase
    let frase5 = ` Informamos que devido ao período de ${diassemserviço} dias,  em que o consumidor já teve os serviços apresentando inoperância inserimos o valor de R$ ${calculo.toFixed(2).replace('.', ',') } `;
...
function gerarFrase6() {
    ...
    // Construa a frase
    let frase6 = `que deverá ser descontado na fatura com vencimento em ${creditfut} , conforme estabelecido no caput do art. 32 do RQUAL.`;
...
function gerarFrase61() {
    ...
    let frase7 = `a fatura com vencimento em ${datajust} deverá ser ajustada e gerada nova conta com vencimento em ${datajustnova} no valor de R$ ${calc2.toFixed(2).replace('.', ',')} ,conforme estabelecido no caput do art. 32 do RQUAL.`;
...
function  gerarEmail () {
    ...
    let frase8 = `  Prazo de envio do boleto por correio de 12 dias úteis e também ficará disponível no App Vivo em até 3 dias úteis. Em caso de dúvidas, envie um e-mail para ${emailcontato}, pois faremos acompanhamento por meio deste.
